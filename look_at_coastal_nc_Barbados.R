...
# The influx observation I got from this method
#has mismatches with the limited on-site knowledge,
#so I decide to look at another location with way more on-site
#knowledge about sargassum influx to check if the method is valid at all.
#I have got the nc file with the same variables for Barbados.


nc_path     <- "data/Barbados.nc" 
...
Barbados <- gadm(country = "Barbados", level = 1, path = tempdir())
Barbados <- aggregate(Barbados)
plot(Barbados)  # quick visual check

# --- Build 1 km seaward buffer in a metric CRS (UTM 19N) ---
utm <- "EPSG:32621"
Barbados_utm <- project(Barbados, utm)
buffer_utm  <- buffer(Barbados_utm, width = 1000)   # 1 km outward
seaward_utm <- erase(buffer_utm, Barbados_utm)      # ring only, drops land
